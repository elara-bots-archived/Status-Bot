...
module.exports = class Bot extends CommandoClient{
    ...
    constructor(info){
        ...
        if(info.embed){
            if(info.embed.off){
            let author = {name: "", icon: ""}, title = `%user% has gone offline!`, color = '#FF0000', description = '', thumbnail = '';
        ...
        if(info.embed.on){
            let author = {name: "", icon: ""}, title = `%user% has come back online!`, color = '#FF000', description = '', thumbnail = '';
        ...
        this.on("ready", async () => {
            if(info.channel){
                ...
                if(!c){
                    ...
                    let perms = ["SEND_MESSAGES", "READ_MESSAGES", "EMBED_LINKS"];
                    if(!c.permissionsFor(c.guild.me).has(perms)){
            ...
            if(n.guild.id === this.guild && this.watch.includes(n.user.id)) {
                if(n.status === "offline"){
                    if(this.role !== ""){
                        ...
                        .setTitle(off.title.replace(/%user%/g, n.user.username).replace(/%status%/g, n.status).replace(/%mention%/g, `<@${n.user.id}>`).replace(/%tag%/g, n.user.tag).replace(/%id%/g, n.user.id).replace(/%icon%/g, n.user.displayAvatarURL()))
                        .setDescription(off.description.replace(/%user%/g, n.user.username).replace(/%status%/g, n.status).replace(/%mention%/g, `<@${n.user.id}>`).replace(/%tag%/g, n.user.tag).replace(/%id%/g, n.user.id).replace(/%icon%/g, n.user.displayAvatarURL()))
                        .setAuthor(off.author.name.replace(/%user%/g, n.user.username).replace(/%status%/g, n.status).replace(/%mention%/g, `<@${n.user.id}>`).replace(/%tag%/g, n.user.tag).replace(/%id%/g, n.user.id).replace(/%icon%/g, n.user.displayAvatarURL()) || n.user.username, off.author.icon.replace(/%icon%/g, n.user.displayAvatarURL()) || n.user.displayAvatarURL())
                        .setColor(off.color)
                        .setTimestamp()
                ...
                if(n.status === 'online' && o.status === 'offline'){
                    let on = this.eon;
                    let e = new MessageEmbed()
                    .setTitle(on.title.replace(/%user%/g, n.user.username).replace(/%status%/g, n.status).replace(/%mention%/g, `<@${n.user.id}>`).replace(/%tag%/g, n.user.tag).replace(/%id%/g, n.user.id).replace(/%icon%/g, n.user.displayAvatarURL()))
                    .setDescription(on.description.replace(/%user%/g, n.user.username).replace(/%status%/g, n.status).replace(/%mention%/g, `<@${n.user.id}>`).replace(/%tag%/g, n.user.tag).replace(/%id%/g, n.user.id).replace(/%icon%/g, n.user.displayAvatarURL()))
                    .setAuthor(on.author.name.replace(/%user%/g, n.user.username).replace(/%status%/g, n.status).replace(/%mention%/g, `<@${n.user.id}>`).replace(/%tag%/g, n.user.tag).replace(/%id%/g, n.user.id).replace(/%icon%/g, n.user.displayAvatarURL()) || n.user.username, on.author.icon.replace(/%icon%/g, n.user.displayAvatarURL()) || n.user.displayAvatarURL())
                    .setColor(on.color)
                    .setTimestamp()
