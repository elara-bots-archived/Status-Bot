    ...
    async run(message){
        ...
        for (const user of this.client.watch){
            ...
            switch(object.presence.status){
                ...
                case "offline": case "invisible": offline.push(object.username); break;
            }
        };
        ...
        if(offline.length) e.addField(`**Offline**`, offline.join('\n'))
        if(online.length) e.addField(`Online`, online.join('\n'))
        if(idle.length) e.addField(`Idle`, idle.join('\n'));
        if(dnd.length) e.addField(`Dnd`, dnd.join('\n'))
